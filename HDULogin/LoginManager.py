...
class LoginManager:
	...
	def _resolve_ip_from_login_page(self):
		...
		self.ip = re.search(r'((1\d{2}|25[0-5]|2[0-4]\d|[1-9]?\d)\.){3}(25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)', self._page_response.text).group()
	...
	def _get_challenge(self):
		...
		params_get_challenge = {
			"callback": "jQuery1124015280105355320628_"+str(int(time.time()*1000)), # This value can be any string, but cannot be absent
